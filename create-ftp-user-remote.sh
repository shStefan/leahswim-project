...
# Скрипт для автоматического создания FTP пользователя на удаленном сервере
...
ssh -i $SSH_KEY -l $USER $SERVER << ENDSSH
    ...
    if id "$FTP_USERNAME" &>/dev/null; then
        ...
        echo "Используйте другой скрипт для изменения пароля или удалите пользователя:"
        echo "  sudo userdel -r $FTP_USERNAME"
        exit 1
    fi
...
echo "   ssh -i ~/.ssh/mainkey main@51.250.97.231 'echo \"$FTP_USERNAME:новый_пароль\" | sudo chpasswd'"
echo ""
